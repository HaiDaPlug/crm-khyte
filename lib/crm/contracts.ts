...
export const createLeadSchema = z.strictObject({
  ...
  followedUpBy: colleague.nullable().describe('Colleague credited for adding this lead; null means explicitly unassigned. You may log for another colleague; this is independent of the account behind the connection.'),
...
export const loggingRules = {
  ...
  attribution: 'This connection acts as the person who approved it, inside their organization; every saved operation is recorded against that account. Who did the work is separate: followedUpBy is explicitly selected and tasks use assignee, so you may log outreach for a colleague. Logging outreach credits the named colleague without changing the existing prospect owner.',
