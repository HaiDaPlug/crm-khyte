...
import { Building2, Plus, Sparkles, User } from 'lucide-react'
import { Modal } from './Modal'
import { ConfirmDialog } from './ConfirmDialog'
import { Button } from './Button'
import { AssigneePicker, ColorSlider, ComboHint, Combobox, Field, comboStatus, inputClass } from './FormFields'
import { useCRMStore } from '@/lib/store'
import { ColleagueId, Priority, Stage } from '@/lib/types'
...
const SPARKLES_ICON = <Sparkles size={15} />
...
export function AddProspectModal({ open, onClose, fromLeadId = null }: AddProspectModalProps) {
  ...
  // The lead this prospect is being promoted from, if any — cleared from the
  // Leads list on submit. Distinct from `fromLeadId`: that's only the initial
  // value; picking a different lead from the search below re-targets this.
  const [leadId, setLeadId] = useState<string | null>(null)
  const [leadQuery, setLeadQuery] = useState('')

  const applyLead = useCallback((lead: (typeof leads)[number]) => {
    setLeadId(lead.id)
    setLeadQuery(lead.companyName)
    setCompanyQuery(lead.companyName)
    setCompanyId(null)
    if (lead.contactName) {
      setContactQuery(lead.contactName)
      setContactId(null)
    }
    setPriority(lead.priority)
    // The lead's connection and source have no home on the Prospect record —
    // fold them into notes rather than silently drop that context on promote.
    const leadCopy = t.crm.newLeadForm
    const extra = [
      lead.connection && `${leadCopy.connection}: ${lead.connection}`,
      lead.source && `${leadCopy.source}: ${lead.source}`,
    ].filter(Boolean)
    setNotesText([lead.notes, ...extra].filter(Boolean).join('\n'))
  }, [t])
  ...
  useEffect(() => {
    ...
    const lead = fromLeadId ? leads.find((l) => l.id === fromLeadId) ?? null : null
    if (lead) {
      applyLead(lead)
      return
    }
    ...
    // Only re-run when the modal opens or the requested lead changes — not on
    // every `leads` mutation, or a promote-in-progress in another tab could
    // reset this form mid-edit.
  ...
  const leadOptions = useMemo(
    () => leads.map((l) => ({ id: l.id, label: l.companyName })),
    [leads]
  )
  ...
  const handleLeadQueryChange = useCallback((q: string) => {
    setLeadQuery(q)
    setLeadId(null)
  }, [])
  ...
  return (
    ...
    >
      {/* Start from a lead — optional; only worth showing if there's anything to pick from */}
      {leads.length > 0 && (
        <div className="border-b border-border-subtle px-4 pb-5 pt-5 sm:px-7 sm:pb-6 sm:pt-6">
          <Field label={copy.fromLead} htmlFor={`${formId}-lead`}>
            <Combobox
              id={`${formId}-lead`}
              icon={SPARKLES_ICON}
              placeholder={copy.fromLeadPlaceholder}
              query={leadQuery}
          ...
          <p className="mt-2 text-[12.5px] text-foreground/60">
            {leadId ? copy.fromLeadHint : leadQuery.trim() && leadOptions.length === 0 ? copy.noLeadMatch : ''}
          </p>
        </div>
      )}
      ...
      {/* Pipeline — selecting a stage is what puts the prospect on the board */}
      <div className="border-b border-border-subtle px-4 py-5 sm:px-7 sm:py-6">
        ...
        <div role="radiogroup" aria-label={copy.pipelineStage} className="flex flex-wrap gap-2 sm:flex-nowrap">
          <button
            type="button"
            role="radio"
