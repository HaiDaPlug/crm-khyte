...
import {
  Plus, Check, Circle, AlertCircle, Calendar, Building2, Pencil,
...
function TaskItem({ task }: { task: Task }) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'flex items-start gap-3 bg-surface px-3 py-3.5 group transition-opacity duration-500 sm:px-4 sm:py-4',
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {task.description && (
          <p className="mt-1 line-clamp-2 text-[13.5px] leading-snug text-foreground/70 sm:line-clamp-1">{task.description}</p>
        )}
        <div className="mt-2 flex flex-wrap items-center gap-x-3.5 gap-y-1.5">
          ...
          {task.assignee && (
            <div className="flex items-center gap-1.5 text-[13.5px] text-foreground/65">
              <span
...
export default function TasksPage() {
  ...
  const totalActive = onPace.length + late.length
    ...
    <>
      ...
      <main className="flex-1 animate-fade-in-up px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-[28px] font-jakarta font-semibold text-foreground tracking-[-0.02em] leading-none sm:text-[30px]">{t.tasks.title}</h2>
            <p className="text-[15px] text-foreground/60 mt-1.5 font-mono tabular-nums">
              {t.tasks.summary(totalActive, completed.length)}
            </p>
          </div>
          <Button onClick={() => setAddTaskOpen(true)} className="h-11 w-full sm:h-[38px] sm:w-auto">
            <Plus size={15} />
            {t.tasks.addTask}
          </Button>
        </div>
